    ...
    <el-dialog :title="title" :visible.sync="openInput" width="500px" append-to-body>
      <el-form ref="formInput" :model="formInput" :rules="rules" label-width="80px">
        <el-form-item label="货架" prop="shelves">
          <el-cascader
            v-model.number="formInput.shelvesCellId"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitFormInput">确 定</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    submitFormInput() {
      this.$refs["formInput"].validate(valid => {
        if (valid) {
          alert(this.formInput.shelvesCellId)
          if(this.formInput.shelvesCellId){
            this.formInput.shelvesCellId=this.formInput.shelvesCellId[this.formInput.shelvesCellId.length-1];
          }
