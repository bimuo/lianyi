        ...
        lazyLoad (node, resolve) {
          ...
          if(root){
            ...
            treeChildrenShelves(data.value).then((response) => {
              ...
              response.map(item=>{
                item.leaf=true;
                item.disabled=(rootThis.productIdByInput!=item.productId && item.productId);
                if(item.label=='0-3'){
                  debugger;
                }
                item.label=((rootThis.productIdByInput==item.productId && item.productId)?item.label+'(' +item.count+')':item.label)
              });
              resolve(response);
            });
          }
